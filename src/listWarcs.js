...
function listWarcs (params = {}) {
  ...
  if (day) {
    Prefix += month + '/' + WARC_FILE_PREFIX + year + month + day
  } else if (month) {
    Prefix += month + '/'
  }
  ...
  if (verbose) {
    console.log({ Bucket, Prefix, date, year, month, day })
  }
  ...
  return (
    s3
      ...
      .then(({ Name, Contents, KeyCount }) => ({
        warcFiles: Contents.map(({ Key }) => ({ Bucket: Name, Key })),
        date: { year, month, day },
        totalFileCount: KeyCount
      }))
  )
}
